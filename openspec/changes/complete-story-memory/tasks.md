...
- [x] 2.6 Add privacy tests proving only matched entries and the configured brief enter `effective_context`, with unmatched memory retained locally.

## 3. Shared Translation and Cache Boundary

- [x] 3.1 Introduce one shared request-context helper that loads project memory and assembles `effective_context` before invoking the normal `Translator` abstraction.
...
- [x] 3.5 Extend translation cache identity with project UUID, memory revision, assembler version, language pair, source content, translator configuration, and user extra context.
- [x] 3.6 Add tests proving matched memory changes the actual direct-LLM prompt, disabled memory preserves current behavior, cache revisions invalidate stale output, and retrieval adds no provider call.
...
- [x] 5.2 Add enablement and bounded Story Brief controls with explicit over-limit validation.
...
- [x] 6.1 Route manual single-page translation through the shared Story Memory boundary and verify it first with memory enabled and disabled.
- [x] 6.2 Route manual multi-page and regular batch translation through the same boundary without changing worker/thread behavior.
- [x] 6.3 Route visible-webtoon and webtoon-batch translation through the same boundary without changing page ordering or rendering behavior.
- [x] 6.4 Redirect the legacy Context Translate action to the shared implementation or remove it only after manual-path parity is demonstrated.
...
- [x] 7.3 Run `git diff --check`, all focused Story Memory tests, both repository test-discovery naming patterns, and targeted import smoke checks.
